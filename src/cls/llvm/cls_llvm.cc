...
static int eval(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  try {
    ...
    CLS_LOG(0, "ERROR: cls_llvm_eval_op(): failed to decode op <%s>", e.what());
   ...
   */
  module = llvm::ParseIR(buf, irError, context);
  if (!module) {
    CLS_LOG(0, "ERROR: could not parse input IR: %s", irError.getMessage().c_str());
    sprintf(msgbuf, "ERROR: could not parse input IR: %s", irError.getMessage().c_str());
  ...
  CLS_LOG(0, "calling %s", ctx.op.function.c_str());
